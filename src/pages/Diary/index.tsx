...
const Diary = () => {
    ...
    const [filteredDiary, setFilteredDiary] = useState<DiaryProps[]>([]);
    ...
    useEffect(() => {
        if (mood) {
            setFilteredDiary(diary.filter(d => d.value === mood));
        } else {
            setFilteredDiary(diary);
        }
    }, [mood, diary]);
            ...
            <div className='min-h-screen'>
                ...
                <div className='p-5 md:py-12 md:px-32 flex flex-row justify-between md:mx-30 lg:mx-60'>
                    <div className="flex w-full bg-leaf justify-center rounded-lg flex-col">
                        ...
                        <MoodFilter moods={moodList.map((item) => item.value)} />
                        
                        {filteredDiary.map((props, index) => (
